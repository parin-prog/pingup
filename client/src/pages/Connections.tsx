  ...
  return <div className="min-h-screen bg-slate-50">
    <div className="max-w-6xl mx-auto p-6">
      ...
      <div className="mb-8">
        ...
        <p className="text-slate-600">Manage your connections and discover new
          connections</p>
      </div>
      ...
      rounded-md p-1 bg-white shadow-sm">
        {
          dataArray.map((tab, i) => (
            <button key={i}
              ...
              className={`flex items-center cursor-pointer px-3 py-1 text-sm rounded-md
              ...
              text-gray-700 px-2 py-0.5 rounded-full">{tab.count}</span>}
            </button>
          ))
        }
      </div>
              ...
              <div className="flex max-sm:flex-col gap-2 mt-4">
                {
                  <button onClick={() => navigate(`/profile/${user._id}`)}
                    className="w-full p-2 text-sm rounded bg-linear-to-r from-indigo-600
